...
export default class PageRender {
  constructor(options) {
    const self = this
    self.id = create_id()
    self.options = _.cloneDeep(pageInitOptions)
  ...
  addWidget(type) {
    const vm = new Vue({
      ...
      created() {
        this.$store.commit('ADD_COMPONENT_TYPE', { [this.id]: { vm: this, options: _.cloneDeep(widgetInitOptions) }})
      },
      methods: {
        handleClick(e) {
          e.stopPropagation()
      ...
      render: function(h) {
        ...
        return (
          <WidgetWrapper id={this.id} nativeOnClick={this.handleClick}>
            <Widget />
          </WidgetWrapper>
        )
      }
